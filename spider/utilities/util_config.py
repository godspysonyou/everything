...
CONFIG_MESSAGE_PATTERN = r"priority=(?P<priority>\d+),\s*keys=(?P<key>.+?),\s*deep=(?P<deep>\d+),\s*(repeat=(?P<repeat>\d+),\s*)?url=(?P<url>.+)$"
...
CONFIG_HEADERS_SET = {item.lower() for item in {\
        ...
        "Accept", "Accept-Encoding", "Accept-Charset", " Acccpt-Language", "If-Modified-Since", "If-None-Match", "Last_Modified"
    }}